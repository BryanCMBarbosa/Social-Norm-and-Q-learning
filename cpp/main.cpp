...
int main(int argc, char** argv)
{
    ...
    if (argc == 4)
    {
        ...
        char *p_end_runs;
        unsigned long long n = strtoull(argv[3], &p_end_runs, 10);

        Simulation s = Simulation(norm, norm_name);
        s.run_generations(n);
    }
    /*
    else if (argc > 6)
    {
        ...
        Simulation s = Simulation(norm, norm_name, payoff_b, payoff_c, alpha);
        s.run_n_runs(n);
    }
    */
    else
